...
app.use("/deleteOrg", (req, res) => {
  ...
  Organization.findOneAndDelete(query, (err, orig) => {
    if (err) {
      ...
      Fund.deleteMany({ org: orig._id }, (err) => {
        if (err) {
          res.render("error", { error: err });
        }
			});
    ...
    if (err) {
      ...
      Contributor.find(filter, (err, all) => {
        if (err) {
          ...
          var filterOrg = { "funds._id": req.body.id };
          var actionOrg = { $set: { "funds.$": result } };

          Organization.findOneAndUpdate(filterOrg, actionOrg, (err) => {
            if (err) {
              res.render("error", { error: err });
            } else {
              Contributor.find((err, allContributors) => {
